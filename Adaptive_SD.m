function [Wopt, IT, ffval,Progr]=Adaptive_SD(P0,N0,W0,prec_loss, prec_grad,eta,maxit,gradtype,Target)
global Q;
global M;
global wkspace;
global full_N;
...
while (stop_criterion)
    ...
    Progr(it,1)= adapt_N;
    Progr(it,2)= adapt_P;
    fprintf(1,'\n---[Adaptive SD] : Iter %d: cost=%.20f, nor_dW=%.20f, tau=%d, P=%d, N=%d',it,current_cost,nor_dW,tau,adapt_P,adapt_N);
    % Perform Descent until the gradient vanishes
    [W,ffval,nor_dW]=Subroutine_SD(adapt_N,adapt_P,W,prec_loss,prec_grad,maxit,gradtype,Target);
...
IT = it;
ffval = current_cost;
fprintf(1,'\n\n [Adaptive SD] : %d iteration to converge with final cost %f\n',it,current_cost);
History_pot_mat =  strcat(wkspace,'/SD/',wkspace,'_History_Pot_progress.mat');
...
History_grad_mat =  strcat(wkspace,'/SD/',wkspace,'_History_Grad_progress.mat');
